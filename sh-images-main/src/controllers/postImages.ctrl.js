...
const postImages = async (req, res) => {
  ...
  if (!idHouse) {
    return res
      ...
      .json({
        data: null,
        success: false,
        message: "idHouse is required",
        error: "idHouse is missing",
      });
  }

  const response = await makeGqlRequest({
    query: CheckOwnershipByIdQuery,
    variables: { id: +idHouse },
    headers,
  });
  ...
  if (response?.data?.sh_ownerships.length === 0) {
    return res
      ...
      .json({
        ...
        error: "The ownership id does not exist",
      });
  }
  ...
  try {
    ...
    console.log("response from makeGqlRequest:", response);
