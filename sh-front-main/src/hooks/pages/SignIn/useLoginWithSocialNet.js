...
import {
  getVarStudentUserFacebook,
  getVarStudentUserGoogle,
  getVarStudentUserGithub,
} from "client/gql/mutations/registerUser/getVarStudentUser";
import { paths } from "config/paths";
import { AUTH_PROVIDERS, USER_CATEGORIES } from "const";
import { useDispatch, useSelector } from "react-redux";
import {
  signInSocialNetAction,
  authSelector,
  clearState,
...
export function useLoginWithSocialNet() {
  ...
  /**************************************************************************************/
  const [
    registerStudentUserWithSocNet,
    { loading, error, data: newStudentUser },
  ] = useMutation(REGISTER_STUDENT_USER_WITH_SOC_NET);

  /**************************************************************************************/

  const dispatch = useDispatch();
  const { isFetching, isSuccess, isError, isAuthenticated, user_category } =
  ...
  const onSubmitLogginWithSocialNet = ({ data, provider }) => {
    ...
    if (AUTH_PROVIDERS.GITHUB === provider) onSubmitStudentUserGithub({ data });
  };
  ...
  const onSubmitStudentUserFacebook = async ({ data }) => {
    ...
    registerStudentUserWithSocNet({ variables })
      ...
      .catch((error) => {
        if (
          error.graphQLErrors.at(0).extensions.code === "constraint-violation"
        ) {
          dispatch(signInSocialNetAction(variables.email));
        }
      });
  ...
  const onSubmitStudentUserGithub = async ({ data }) => {
    ...
    const variables = getVarStudentUserGithub(data);
  
    try {
      const email = await getEmailFromGithub(access_token);
      variables.email = email;
      const response = await registerStudentUserWithSocNet({ variables });
      dispatch(signInSocialNetAction(variables.email));
    } catch (error) {
      console.log(error);
